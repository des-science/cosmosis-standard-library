...
class DESY5SNLikelihood(GaussianLikelihood):
    ...
    def build_data(self):
        ...
        # M_sim is absolute magnitude used when simulating the datavector 
        # We only need this value for the simulated likelihood 
        M_sim = self.options.get_double("M_sim", default=0.0)
        if M_sim != 0: print ("use M_sim to convert the simulated modulus to apparent mag")
        ...
        # Simulated DES Y5 SN data is distance modulus, so we need to convert it to the apparent magnitude
        # by adding the absolute magnitude. This is only for simulated data vector. 
        self.m_obs = self.mu_obs + M_sim
    ...
    def extract_theory_points(self, block):
        ...
        zhel = self.zHEL
        # apparent magnitude 
        theory_ynew = 5.0*np.log10((1.0+zcmb)*(1.0+zhel)*np.atleast_1d(f(zcmb))) + 25.0

        # convert mag to modulus 
