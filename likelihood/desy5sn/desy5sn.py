...
# default files from https://github.com/des-science/DES-SN5YR/tree/main/4_DISTANCES_COVMAT 
# if file names are not specified in ini file, use default files. 
...
class DESY5SNLikelihood(GaussianLikelihood):
    ...
    def build_data(self):
        ...
        if 'MUERR' in data.keys(): self.mu_obs_err = data['MUERR'][self.ww]
        elif 'MUERR_FINAL' in data.keys(): self.mu_obs_err = data['MUERR_FINAL'][self.ww]
        else: print ('cannot find the MU ERROR column')
